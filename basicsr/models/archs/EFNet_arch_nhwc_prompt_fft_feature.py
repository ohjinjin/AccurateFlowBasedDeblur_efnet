...
#         print("CHCHCHCH:::::", merged_out_high.shape) # torch.Size([8, 256, 64, 64])
        ...
        prompt_weights = F.softmax(merged_out, dim=1)  # will be shared to both PGM and MFGM
...
class CustomUpBlock(nn.Module):  # in_size = 2*out_size
    def __init__(self, in_size, out_size, relu_slope, prompt_len=3, prompt_size=16, num_heads=None, ffn_expansion_factor=2.66, bias=False, num_blocks=[1, 4, 4], LayerNorm_type='WithBias'):
        ...
        self.prompt = PromptMFeatureGenBlock(prompt_dim=in_size, prompt_len=prompt_len, prompt_size=prompt_size, lin_dim=in_size, relu_slope=relu_slope)
...
#         print("CHECK JINJIN out_dec_prev_level::::", out_dec_prev_level.shape, "out_enc_curr_level:::: ", out_enc_curr_level.shape)
        ...
        # 1. PromptGenBlock 처리
        out_dec_prev_level = self.prompt(out_dec_prev_level, motion_dec_prev_level)  # equation (2)에서 F_l대신 motion feature인 G_l로 입력 바꿔줘야하고 함수정의자체에서도 5개의 앙상블링하게끔 마저 수정필요함.
